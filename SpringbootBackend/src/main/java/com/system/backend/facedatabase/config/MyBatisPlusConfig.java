package com.system.backend.facedatabase.config;
...
public class MyBatisPlusConfig implements WebMvcConfigurer {
    ...
    @Primary：如果IOC容器中出现了相同类型的bean对象  优先使用这个
    @ConfigurationProperties(prefix = "spring.datasource.druid"):读取springboot核心配置文件中的属性值，
    前缀必须为 spring.datasource.druid
     */
...
//    @Value("${spring.servlet.multipart.location}")
    ...
    public void addResourceHandlers(ResourceHandlerRegistry registry) {
        //注意！！！！！！！addResourceLocations里一定要以/结尾
        registry.addResourceHandler("/database/**")
        .addResourceLocations("file:" + databaseDirectory);
    }
